...
from app.models import store, order, order_item, models
...
@router.get("/api/orders/my", response_model=List[OrderOut])
def list_user_orders(user_data: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    user_id = user_data.get("id_user") or user_data.get("user_id")
    if not user_id:
        raise HTTPException(status_code=401, detail="Usuário não autenticado")
    orders = db.query(order.Order).filter(order.Order.id_user == user_id, order.Order.status == "paid").all()
    return orders
...
def finalize_order_logic(id: int, db: Session):
    ...
    for item in items:
        ...
        loja.balance += Decimal(item.subtotal)
